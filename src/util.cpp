...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "PePeCoin" is a composite category enabling all PePeCoin-related debug output
            if(ptrCategory->count(string("PePeCoin"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\PePeCoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\PePeCoinCore
    // Mac: ~/Library/Application Support/PePeCoinCore
    // Unix: ~/.PePeCoincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "PePeCoinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/PePeCoinCore";
#else
    // Unix
    return pathRet / ".PePeCoincore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty PePeCoin.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override PePeCoin.conf
